...
def log_startup_info():
    ...
    logger.warning(f"   Task TTL: {TASK_TTL_HOURS}h (3 days)")
    logger.warning(f"   Recovery: max_retries={MAX_TASK_RETRIES}, delay={RETRY_DELAY_SECONDS}s")
    logger.warning(f"   Webhook: interval={WEBHOOK_BACKGROUND_INTERVAL_SECONDS}s, max_retries={WEBHOOK_MAX_RETRY_ATTEMPTS}")
    logger.warning(f"   Cleanup: interval={CLEANUP_INTERVAL_SECONDS}s (1 hour)")
    ...
    logger.warning("   ✓ Variable TTL (1h - 90 days)")
    logger.warning("   ✓ Custom webhook/recovery intervals")
    logger.warning("   ✓ Flexible client_meta limits")
    logger.warning("")
