...
| `INTERNAL_BASE_URL` | `http://video-processor:5001` | Внутренний базовый URL для генерации ссылок в webhook/background контекстах (когда отсутствует request context). Используется для `check_status_url` в async ответах и для `download_url`/`metadata_url` в webhook payload. |
