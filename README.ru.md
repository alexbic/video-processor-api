...
- 🖼️ **Авто Thumbnails** - автоматическая генерация превью из Shorts видео
- ✂️ **Нарезка видео** - по таймкодам с конвертацией в Shorts
- 🎵 **Извлечение аудио** - из видеофайлов
- 📡 **Webhooks** - уведомления о завершении с автоматическими повторами (каждые 15 мин)
- 🎯 **Кастомные Webhook Headers** - заголовки для аутентификации на уровне запроса
- 🔄 **Webhook State Tracking** - унифицированное состояние webhook в metadata.json
- ⏰ **Отслеживание истечения файлов** - поле `expires_at` показывает когда файлы будут удалены
