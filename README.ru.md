...
curl -X POST http://localhost:5001/process_video \
  ...
  -d '{
    ...
    "client_meta": {
      "youtube_title": "Невероятное видео #Shorts",
      "tiktok_title": "Смотри это!"
    }
  }'
```
...
- `make_short` - конверсия в Shorts формат с текстовыми оверлеями; поддерживает `start_time`/`end_time` для автоматической нарезки; авто-генерация thumbnails
- `extract_audio` - извлечение аудиодорожки с автоматическим чанкингом для Whisper API

**Система Text Items (Универсальная):**
Массив `text_items` позволяет гибко управлять текстовыми оверлеями:
- **text** - Текстовый контент (обязательно)
- **fontfile** - Имя файла шрифта из `/app/fonts/` (опционально, по умолчанию DejaVu Sans)
- **fontsize** - Размер шрифта в пикселях (по умолчанию: 60)
- **fontcolor** - Цвет (по умолчанию: white) - поддерживает hex, именованные цвета и альфа (@0.5)
- **x**, **y** - Позиция (поддерживает выражения типа `(w-text_w)/2` для центрирования)
- **start**, **end** - Время отображения в секундах (по умолчанию: 0-5)
- **max_lines** - Максимум строк для переноса текста (по умолчанию: 3)
- **text_align** - Выравнивание текста: left, center, right (по умолчанию: center)
...
**Генерация Thumbnail:**
Операция `make_short` автоматически генерирует thumbnail (JPEG изображение) из обработанного видео:
- **По умолчанию**: `generate_thumbnail: true` (включено по умолчанию)
- **Timestamp**: `thumbnail_timestamp: 0.5` (извлечение кадра на 0.5 секунде от обрезанного видео)
- **Качество**: Высокое качество JPEG (q:v 2)
- **Разрешение**: Такое же как у видео (1080x1920 для Shorts)
- **Использование**: Идеально для кастомных thumbnails YouTube/TikTok с отображением текстовых оверлеев
- **Отключить**: Установите `generate_thumbnail: false` чтобы пропустить создание thumbnail
